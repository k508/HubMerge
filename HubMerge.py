...
filtered_csv = pd.read_csv("output.csv", usecols=['Phone Number',
...
with open('filtered_csv.csv') as file:
    ...
    for merge_fields in reader:
        if merge_fields[11] != '':
            appointment_datetime_object = datetime.strptime(merge_fields[11], '%Y-%m-%d')
            appointment_datetime_format = datetime.strftime(appointment_datetime_object, '%d-%m-%Y')
        else:
                appointment_datetime_format = merge_fields[11]
        if merge_fields[5] != '':
            conf_datetime_object = datetime.strptime(merge_fields[5], '%Y-%m-%d')
            conf_datetime_format = datetime.strftime(conf_datetime_object, '%d-%m-%Y')
        else:
